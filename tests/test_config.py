...
@pytest.mark.parametrize("tls_config,expected_error", [
    # Wrong type: tls config is a string instead of a dictionary
    (
        "not-a-dict",
        "TLS configuration must be a dictionary"
    ),
    # ca_cert missing when enabled is True
    (
        {'enabled': True},
        "requires 'ca_cert' when 'enabled' is true"
    ),
...
@pytest.mark.parametrize("tls_config", [
    # No tls section
    None,
